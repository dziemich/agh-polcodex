...
import pl.kis.agh.compilers.parsing.AntrlParsingFacade
...
class MappingTest {
    ...
    @test fun mapSimpleFileWithoutPositions() {
        val code = """var a = 1 + 2
                     |a = 7 * (2 / 3)""".trimMargin("|")
        val ast = AntrlParsingFacade.parse(code).root!!.toAst()
        val expectedAst = SandyFile(
          listOf(
            VarDeclaration(
              "a",
              SumExpression(
    ...
    @test fun mapSimpleFileWithPositions() {
        val code = """var a = 1 + 2
                     |a = 7 * (2 / 3)""".trimMargin("|")
        val ast = AntrlParsingFacade.parse(code).root!!.toAst(considerPosition = true)
        val expectedAst = SandyFile(
          listOf(
            VarDeclaration(
              "a",
